...
func (p *ProjectRoleTemplateBindingSuite) Test_PrivilegeEscalation() {
	...
	for i := range tests {
		...
		p.Run(test.name, func() {
			...
			router.Kind("ProjectRoleTemplateBinding").Group(management.GroupName).Type(&apisv3.ProjectRoleTemplateBinding{}).Handle(validator)
			bytesBuffer := bytes.NewBuffer([]byte{})
			err := json.NewEncoder(bytesBuffer).Encode(v1.AdmissionReview{Request: &req})
			p.NoError(err, "unable to encode admission review")
			httpRequest := httptest.NewRequest("get", "http://localhost", bytesBuffer)
			httpResponse := httptest.NewRecorder()

			// send request to validator.Admit
			router.ServeHTTP(httpResponse, httpRequest)

			// extract the response, validate it worked as expected
			responseCode := httpResponse.Code
			p.GreaterOrEqualf(responseCode, 200, "response code was below 200, expected above 200")
			p.Lessf(responseCode, 300, "response code was above 300 indicating an error, did not expect error")
			review := &v1.AdmissionReview{}
			err = json.NewDecoder(httpResponse.Body).Decode(review)
			p.NoError(err, "unable to decode admission response")
			response := review.Response
			p.NotNil(response, "admission response was nil but was not expected to be")
			if response.Allowed != test.allowed {
				p.Failf("Response was incorrectly validated", "Wanted response.Allowed = '%v' got %v: result=%+v", test.allowed, response.Allowed, response.Result)
			}
		})
	}
}
...
func createPRTBRequest(t *testing.T, oldPRTB, newPRTB *apisv3.ProjectRoleTemplateBinding, username string) v1.AdmissionRequest {
	t.Helper()
