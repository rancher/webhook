...
func (c *ClusterRoleTemplateBindingSuite) Test_PrivilegeEscalation() {
	...
	for i := range tests {
		...
		c.Run(test.name, func() {
			...
			router.Kind("ClusterRoleTemplateBinding").Group(management.GroupName).Type(&apisv3.ClusterRoleTemplateBinding{}).Handle(validator)
			bytesBuffer := bytes.NewBuffer([]byte{})
			err := json.NewEncoder(bytesBuffer).Encode(v1.AdmissionReview{Request: &req})
			c.NoError(err, "unable to encode admission review")
			...
			c.GreaterOrEqualf(responseCode, 200, "response code was below 200, expected above 200")
			c.Lessf(responseCode, 300, "response code was above 300 indicating an error, did not expect error")
			review := &v1.AdmissionReview{}
			err = json.NewDecoder(httpResponse.Body).Decode(review)
			c.NoError(err, "unable to decode admission response")
			response := review.Response
			c.NotNil(response, "admission response was nil but was not expected to be")
			if response.Allowed != test.allowed {
				c.Failf("Response was incorrectly validated", "Wanted response.Allowed = '%v' got %v: result=%+v", test.allowed, response.Allowed, response.Result)
			}
		})
	}
}
...
func createCRTBRequest(t *testing.T, oldCRTB, newCRTB *apisv3.ClusterRoleTemplateBinding, username string) v1.AdmissionRequest {
	t.Helper()
