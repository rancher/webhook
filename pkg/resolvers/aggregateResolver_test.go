	...
	tests := []struct {
	...
	}{
		{
			...
			resolvers: func(t *testing.T) ([]validation.AuthorizationRuleResolver, Rules) {
				...
				resolver.EXPECT().VisitRulesFor(context.Background(), testUser, testNameSpace, gomock.Any()).
				...
				resolver.EXPECT().GetRoleReferenceRules(context.Background(), gomock.Any(), gomock.Any()).Return(expectedRules, nil)
		...
		},
		{
			...
			resolvers: func(t *testing.T) ([]validation.AuthorizationRuleResolver, Rules) {
				...
				resolver := mocks.NewMockAuthorizationRuleResolver(gomock.NewController(t))
				resolver.EXPECT().VisitRulesFor(context.Background(), gomock.Any(), testNameSpace, gomock.Any()).
					DoAndReturn(func(_ context.Context, _ user.Info, _ string, visitor func(source fmt.Stringer, rule *rbacv1.PolicyRule, err error) bool) bool {
						visitor(nil, nil, errNotFound)
						return true
					})
				resolver2 := mocks.NewMockAuthorizationRuleResolver(gomock.NewController(t))
				resolver2.EXPECT().VisitRulesFor(context.Background(), gomock.Any(), testNameSpace, gomock.Any()).
					DoAndReturn(func(_ context.Context, _ user.Info, _ string, visitor func(source fmt.Stringer, rule *rbacv1.PolicyRule, err error) bool) bool {
						visitor(nil, nil, errNotFound)
						return true
					})
		...
		},
		{
			...
			resolvers: func(t *testing.T) ([]validation.AuthorizationRuleResolver, Rules) {
				...
				resolver.EXPECT().GetRoleReferenceRules(context.Background(), gomock.Any(), gomock.Any()).Return(expectedRules, nil)
				resolver2 := mocks.NewMockAuthorizationRuleResolver(gomock.NewController(t))
				resolver2.EXPECT().VisitRulesFor(context.Background(), testUser, testNameSpace, gomock.Any()).
					DoAndReturn(func(_ context.Context, _ user.Info, _ string, visitor func(source fmt.Stringer, rule *rbacv1.PolicyRule, err error) bool) bool {
						for _, rule := range expectedRules {
							visitor(nil, &rule, nil)
						}
				...
				resolver2.EXPECT().GetRoleReferenceRules(context.Background(), gomock.Any(), gomock.Any()).Return(nil, nil)
				return []validation.AuthorizationRuleResolver{resolver, resolver2}, expectedRules
			},
		},
		{
			...
			resolvers: func(t *testing.T) ([]validation.AuthorizationRuleResolver, Rules) {
				...
				resolver.EXPECT().GetRoleReferenceRules(context.Background(), gomock.Any(), gomock.Any()).Return(expectedRules1, nil)
				resolver2 := mocks.NewMockAuthorizationRuleResolver(gomock.NewController(t))
				resolver2.EXPECT().VisitRulesFor(context.Background(), testUser, testNameSpace, gomock.Any()).
					DoAndReturn(func(_ context.Context, _ user.Info, _ string, visitor func(source fmt.Stringer, rule *rbacv1.PolicyRule, err error) bool) bool {
				...
				resolver2.EXPECT().GetRoleReferenceRules(context.Background(), gomock.Any(), gomock.Any()).Return(expectedRules2, nil)
